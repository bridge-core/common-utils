...
picomatch.parse = (pattern, options) => {
  ...
  return parse_1(pattern, __spreadProps(__spreadValues({}, options), { fastpaths: false }));
};
...
function isMatch(path2, pattern) {
  if (Array.isArray(pattern))
    return pattern.some((p) => isMatch(path2, p));
  return picomatch$1(pattern)(path2);
}
...
function walkObject(path2, obj, onReach) {
...
function hasAnyPath(obj, paths) {
  let hasPath = false;
  for (const path2 of paths) {
    walkObject(path2, obj, () => hasPath = true);
    if (hasPath)
      return true;
  }
  return false;
